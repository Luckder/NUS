...
class BankEvent extends Event {
   ...
   * implemented as enum, but we have not covered
   * that yet.
   */
   ...
   */
  public BankEvent(
      int eventType, double time, int customerId, double serviceTime, boolean[] available) {
    // Call the constructor BankEvent(int, double, int)
    this(eventType, time, customerId);
  ...
  public String toString() {
    ...
    if (this.eventType == BankEvent.ARRIVAL) {
      ...
      str =
          String.format(
              ": Customer %d service begins (at Counter %d)", this.customerId, this.counterId);
    } else if (this.eventType == BankEvent.SERVICE_END) {
      str =
          String.format(
              ": Customer %d service ends (at Counter %d)", this.customerId, this.counterId);
    } else if (this.eventType == BankEvent.DEPARTURE) {
      str = String.format(": Customer %d departs", this.customerId);
    }
  ...
  public Event[] simulate() {
    if (this.eventType == BankEvent.ARRIVAL) {
      ...
      if (counter == -1) {
        ...
        // should depart.
        return new Event[] {new BankEvent(BankEvent.DEPARTURE, this.getTime(), this.customerId)};
      } else {
        // Else, the customer should go the first
        // available counter and get served.
        return new Event[] {
          new BankEvent(
              BankEvent.SERVICE_BEGIN,
              this.getTime(),
              this.customerId,
              this.serviceTime,
              counter,
