...
public class GameTree {
    ...
    int findValue() {
        ...
        return minimax(root, Player.ONE);
    }

    // Helper fucntion
    private int minimax(TreeNode node, Player player) {
        if (node.leaf) {
            return node.value;
        }
        ...
        if (player == Player.ONE) {
            ...
            for (int i = 0; i < node.numChildren; i++) {
                ...
                int childValue = minimax(node.children[i], Player.TWO);
            ...
            for (int i = 0; i < node.numChildren; i++) {
                ...
                int childValue = minimax(node.children[i], Player.ONE);
